    ...
    currentPath: function () {
      ...
      Object.keys(this.$route.query).forEach(function (prop) {
        ...
        if (queryVal.includes(",")) {
          let list = queryVal.split(",").map(value => value.toLowerCase());
          queryVal = [...new Set(list)].join(",");
          console.log("QV is now... " + queryVal);
        }
        if (queryVal) {
      ...
      console.log("RV: " + JSON.stringify([title, queryParams]))
